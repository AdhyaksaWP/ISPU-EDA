...
def extract_station_id(filename):
    """
    Extract station ID from filename based on location name (not dki number).
    This fixes the DKI4/DKI5 swap in the raw filenames.
    
    ISPU Reference (source of truth):
    - DKI1: Bundaran HI (Central)
    - DKI2: Kelapa Gading (North)
    - DKI3: Jagakarsa (South)
    - DKI4: Kebon Jeruk (West)
    - DKI5: Lubang Buaya (East)
    """
    basename = Path(filename).name.lower()
    
    # Map by location name to correct DKI label
    location_to_station = {
        'bundaranhi': 'DKI1',
        'kelapagading': 'DKI2',
        'jagakarsa': 'DKI3',
        'kebonjeruk': 'DKI4',      # File incorrectly labeled as dki5
        'lubangbuaya': 'DKI5'       # File incorrectly labeled as dki4
    }
    
    for location, station in location_to_station.items():
